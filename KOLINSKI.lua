...
addcmd('cmds', {'help'}, function(args, speaker)
	...
	if CMDs then
		for _, cmdData in pairs(CMDs) do
			...
			if cmdData.NAME then
				name = cmdData.NAME
			elseif cmdData.Aliases and #cmdData.Aliases > 0 then
				name = cmdData.Aliases[1]
			end
	...
	ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 20)
	FRAME:TweenPosition(UDim2.new(0.5, -200, 0.5, -200), "Out", "Quart", 0.5, true)
